...
(see figure). The authors achieve this by resampling $\hat{f}$ on the grid
...
"column" of the parallelogram $P_j$ containing the sheared window (see figure),
that can be computed with a $O(n^2 \log n)$ complexity in a sufficiently exact
approximation.
...
each orientation $\theta_l$ (see figure). Thus, the wrapped, localized data are
\begin{equation*}
